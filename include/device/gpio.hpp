...
class gpio_pin {
    BOOST_STATIC_ASSERT_MSG(PIN < 8, "PIN value in gpio_pin must be a number 0-7");
    static const uint8_t PIN_MASK = 1 << PIN;   // Pin mask for use in all the functions
public:
    ...
    static void configure(const PinConfig config) {
        typedef reg8_t<PORT::PIN0CTRL::ADDRESS + PIN> THIS_PINCTRL;    // local typedef for offset of PINCTRL
        THIS_PINCTRL::write(config);
    }

    template <uint8_t INT_NUM>
    static void enable_interrupt() {
        BOOST_STATIC_ASSERT_MSG(INT_NUM <= 1, "GPIO interrupt must be 0 or 1");
        typedef reg8_t<PORT::INTMASK_BASE_ADDRESS + INT_NUM> THIS_INTMASK;    // local typedef for offset of interrupt
